...
public class Rail : StackableBase
{
    ...
    public override ILinkable upperNeighbor => GetNeighbor(transform.position + (Vector3.forward * MyGrid._instance.cellSize));
    ...
    {
        this.next = next;
        Railway.Instance.AddRail(next as Rail); // 确保新轨道被添加到系统
